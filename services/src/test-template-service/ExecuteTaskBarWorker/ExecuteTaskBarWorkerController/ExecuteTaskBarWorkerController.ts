...
export interface IExecuteTaskBarWorkerController {
 ...
 *
 */
export class ExecuteTaskBarWorkerController implements IExecuteTaskBarWorkerController {
  /**
   *
   */
  constructor(private readonly executeTaskBarWorkerService: IExecuteTaskBarWorkerService) {}

  /**
   *
   */
  public async executeTasks(sqsEvent: SQSEvent): Promise<SQSBatchResponse> {
    const logCtx = 'ExecuteTaskBarWorkerController.executeTasks'
  ...
  > {
    const logCtx = 'ExecuteTaskBarWorkerController.executeTaskSafe'
    ...
    const executeTaskResult = await this.executeTaskBarWorkerService.executeTask(incomingEvent)
  ...
  private parseInputEvent(sqsRecord: SQSRecord): Success<unknown> | Failure<'InvalidArgumentsError'> {
    const logCtx = 'ExecuteTaskBarWorkerController.parseInputEvent'
