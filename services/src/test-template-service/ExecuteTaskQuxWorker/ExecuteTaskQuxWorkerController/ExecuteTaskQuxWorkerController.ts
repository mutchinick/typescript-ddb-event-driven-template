...
export interface IExecuteTaskQuxWorkerController {
 ...
 *
 */
export class ExecuteTaskQuxWorkerController implements IExecuteTaskQuxWorkerController {
  /**
   *
   */
  constructor(private readonly executeTaskQuxWorkerService: IExecuteTaskQuxWorkerService) {}

  /**
   *
   */
  public async executeTasks(sqsEvent: SQSEvent): Promise<SQSBatchResponse> {
    const logCtx = 'ExecuteTaskQuxWorkerController.executeTasks'
  ...
  > {
    const logCtx = 'ExecuteTaskQuxWorkerController.executeTaskSafe'
    ...
    const executeTaskResult = await this.executeTaskQuxWorkerService.executeTask(incomingEvent)
  ...
  private parseInputEvent(sqsRecord: SQSRecord): Success<unknown> | Failure<'InvalidArgumentsError'> {
    const logCtx = 'ExecuteTaskQuxWorkerController.parseInputEvent'
