...
export interface IListJobEventsApiLambdaConstructProps {
  httpApi: HttpApi
  dynamoDbTable: Table
}
 ...
 *
 */
export class ListJobEventsApiLambdaConstruct extends Construct {
  /**
   *
   */
  constructor(scope: Construct, id: string, props: IListJobEventsApiLambdaConstructProps) {
    super(scope, id)
    const lambdaFunc = this.createListJobEventsApiLambdaFunction(this, id, props.dynamoDbTable)
    this.createListJobEventsApiLambdaIntegration(id, lambdaFunc, props.httpApi)
  }

  /**
   *
   */
  private createListJobEventsApiLambdaFunction(scope: Construct, id: string, dynamoDbTable: Table): NodejsFunction {
   ...
   *
   */
  private createListJobEventsApiLambdaIntegration(id: string, lambdaFunc: NodejsFunction, httpApi: HttpApi): void {
    ...
    httpApi.addRoutes({
      path: '/api/v1/test-template-service/listJobEvents',
