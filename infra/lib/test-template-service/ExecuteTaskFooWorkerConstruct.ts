...
export interface IExecuteTaskFooWorkerConstructProps {
  dynamoDbTable: Table
  eventBus: EventBus
}
 ...
 *
 */
export class ExecuteTaskFooWorkerConstruct extends Construct {
  /**
   *
   */
  constructor(scope: Construct, id: string, props: IExecuteTaskFooWorkerConstructProps) {
    super(scope, id)
    const dlq = this.createExecuteTaskFooWorkerDlq(this, id)
    const queue = this.createExecuteTaskFooWorkerQueue(this, id, dlq)
    this.createExecuteTaskFooWorkerFunction(this, id, props.dynamoDbTable, queue)
    this.createExecuteTaskFooWorkerRoutingRule(this, id, props.dynamoDbTable, props.eventBus, queue)
  }

  /**
   *
   */
  private createExecuteTaskFooWorkerDlq(scope: Construct, id: string): Queue {
   ...
   *
   */
  private createExecuteTaskFooWorkerQueue(scope: Construct, id: string, dlq: Queue): Queue {
   ...
   *
   */
  private createExecuteTaskFooWorkerFunction(
    scope: Construct,
    id: string,
    dynamoDbTable: Table,
    queue: Queue,
    ...
    const lambdaFunc = new NodejsFunction(scope, lambdaFuncName, {
      ...
      entry: join(servicesRoot, 'src/test-template-service/ExecuteTaskFooWorker/handler/handler.ts'),
      handler: 'handler',
      environment: {
   ...
   *
   */
  private createExecuteTaskFooWorkerRoutingRule(
    scope: Construct,
    id: string,
    dynamoDbTable: Table,
    eventBus: EventBus,
