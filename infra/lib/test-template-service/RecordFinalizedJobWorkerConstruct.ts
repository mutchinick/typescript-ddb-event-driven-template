...
export interface IRecordFinalizedJobWorkerConstructProps {
  dynamoDbTable: Table
  eventBus: EventBus
}
 ...
 *
 */
export class RecordFinalizedJobWorkerConstruct extends Construct {
  /**
   *
   */
  constructor(scope: Construct, id: string, props: IRecordFinalizedJobWorkerConstructProps) {
    super(scope, id)
    const dlq = this.createRecordFinalizedJobWorkerDlq(this, id)
    const queue = this.createRecordFinalizedJobWorkerQueue(this, id, dlq)
    this.createRecordFinalizedJobWorkerFunction(this, id, props.dynamoDbTable, queue)
    this.createRecordFinalizedJobWorkerRoutingRule(this, id, props.dynamoDbTable, props.eventBus, queue)
  }

  /**
   *
   */
  private createRecordFinalizedJobWorkerDlq(scope: Construct, id: string): Queue {
   ...
   *
   */
  private createRecordFinalizedJobWorkerQueue(scope: Construct, id: string, dlq: Queue): Queue {
   ...
   *
   */
  private createRecordFinalizedJobWorkerFunction(
    scope: Construct,
    id: string,
    dynamoDbTable: Table,
    queue: Queue,
    ...
    const lambdaFunc = new NodejsFunction(scope, lambdaFuncName, {
      ...
      entry: join(servicesRoot, 'src/test-template-service/RecordFinalizedJobWorker/handler/handler.ts'),
      handler: 'handler',
      environment: {
   ...
   *
   */
  private createRecordFinalizedJobWorkerRoutingRule(
    scope: Construct,
    id: string,
    dynamoDbTable: Table,
    eventBus: EventBus,
