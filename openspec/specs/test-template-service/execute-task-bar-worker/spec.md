...
Process each accepted `STEP_PROCESSED_EVENT` by deriving and persisting exactly one `TASK_BAR_EXECUTED_EVENT` for the same job identifier, while treating duplicate reprocessing as a safe idempotent outcome and separating permanent input rejection from transient infrastructure failure.

## Requirements

### Requirement: Accept only a valid step-processed domain event as the trigger contract

The ExecuteTaskBarWorker SHALL begin processing only when it receives a valid `STEP_PROCESSED_EVENT` envelope whose payload satisfies the step-processed domain event schema.
...
- **THEN** the worker does not emit a `TASK_BAR_EXECUTED_EVENT`
- **THEN** the worker acknowledges the message instead of requeueing it

### Requirement: Transform step-processed input into a task-bar-executed output event

The ExecuteTaskBarWorker SHALL transform each accepted `STEP_PROCESSED_EVENT` into one `TASK_BAR_EXECUTED_EVENT` that preserves the same `jobId` and sets `executed` to the literal boolean `true`.

#### Scenario: Accepted step-processed event produces the canonical task-bar-executed event

- **WHEN** the worker processes a valid `STEP_PROCESSED_EVENT` for a job identifier
- **THEN** it emits a `TASK_BAR_EXECUTED_EVENT` with the same `jobId`
...
- **THEN** the emitted `TASK_BAR_EXECUTED_EVENT.eventData.jobId` is exactly the same value as the input event `jobId`
...
The ExecuteTaskBarWorker SHALL guarantee that reprocessing the same logical step-processed event does not create a second task-bar-executed record and completes safely as an idempotent success from the workflow perspective.

#### Scenario: Duplicate task execution is acknowledged without creating a second event

- **GIVEN** a `TASK_BAR_EXECUTED_EVENT` already exists for `jobId` `job-123456`
- **WHEN** the worker processes another valid `STEP_PROCESSED_EVENT` for the same `jobId`
- **THEN** the event store rejects the duplicate task-bar-executed publish attempt with `DuplicateEventError`
- **THEN** the worker does not create a second `TASK_BAR_EXECUTED_EVENT`
...
- **THEN** at most one `TASK_BAR_EXECUTED_EVENT` is persisted for that logical job step
...
The ExecuteTaskBarWorker SHALL classify business and schema rejections as non-transient and infrastructure failures as transient so the queue can distinguish acknowledge-only outcomes from retryable outcomes.
...
- **WHEN** the event store reports `DuplicateEventError` while publishing the task-bar-executed event
